...
function itemRows(data) {
  ...
  return items.map((item) => row('items', 'work', formatTime(item.create_time), item.metrics?.view_count ?? '', item.description || '', String(item.id || ''), {
...
function billboardRows(data) {
  const list = data?.billboard_data?.element_list || [];
  return list.map((item) => row('billboard', 'topic', '', item.statistics_data?.hot_value || '', item.base_data?.title || '', item.base_data?.billboard_id || '', {
